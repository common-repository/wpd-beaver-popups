...
class PopupHelper {
	...
	public function registerHooks() {
		...
		add_filter ( 'manage_edit-' . Plugin::$config->plugin_slug . '_columns', [ __CLASS__,  'removePostTypeFromQuickAndBulkEdit' ] );
	}
		...
		if ( is_single() || is_page() ) {
			...
			if ( class_exists( '\FLThemeBuilder' ) ) {
				if ( \FLThemeBuilder::has_layout() ) {
					...
					foreach ( \FLThemeBuilderLayoutData::get_current_page_layout_ids() as $post_id ) {
						$themer_click_triggered_popups = (array) get_metadata( 'post', $post_id, Plugin::$config->wp_options[ 'public' ]['CLICK_TRIGGER_POPUPS'], true );
			...
			foreach ( $click_triggered_popups as $click_triggered_popup ) {
				$popup                          = new \stdClass();
				if ( Util::isPopup( $click_triggered_popup ) ) {
